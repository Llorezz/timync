...
<head>
    ...
    <title>{{ $negocio->nombre_negocio ?? $negocio->name }}</title>
...
<body>
<div class="hero">
    @if($negocio->fotos_galeria && count($negocio->fotos_galeria) > 0)
        <div class="hero-bg" style="background-image:url('{{ asset('storage/' . $negocio->fotos_galeria[0]) }}')"></div>
    @elseif($negocio->foto_portada)
        <div class="hero-bg" style="background-image:url('{{ asset('storage/' . $negocio->foto_portada) }}')"></div>
    @else
    ...
    <div class="hero-content">
        @if($negocio->foto_portada)
            <img src="{{ asset('storage/' . $negocio->foto_portada) }}" class="hero-avatar" alt="">
        @else
            <div class="hero-avatar-placeholder">{{ strtoupper(substr($negocio->nombre_negocio ?? $negocio->name, 0, 1)) }}</div>
        @endif
        <h1>{{ $negocio->nombre_negocio ?? $negocio->name }}</h1>
        <div class="hero-meta">
            @if($negocio->ciudad)<span>📍 {{ $negocio->ciudad }}</span>@endif
            @if($negocio->horario_apertura && $negocio->horario_cierre)<span>🕐 {{ $negocio->horario_apertura }} – {{ $negocio->horario_cierre }}</span>@endif
            @if($negocio->telefono_negocio)<span>📞 {{ $negocio->telefono_negocio }}</span>@endif
        </div>
        <div class="social-links">
            @if($negocio->instagram)
            <a href="https://instagram.com/{{ ltrim($negocio->instagram,'@') }}" target="_blank" class="social-btn">
            ...
            @endif
            @if($negocio->facebook)
            <a href="{{ $negocio->facebook }}" target="_blank" class="social-btn">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="white"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                Facebook
            </a>
            @endif
            @if($negocio->whatsapp_negocio)
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/','',$negocio->whatsapp_negocio) }}" target="_blank" class="social-btn">
    ...
    <div class="grid">
        <div>
            ...
            <div class="card"><div class="card-body">
                ...
                <p style="color:#475569;font-size:14px;line-height:1.8;">{{ $negocio->descripcion_negocio }}</p>
            </div></div>
            @endif
            @if($negocio->fotos_galeria && count($negocio->fotos_galeria) > 0)
            <div class="card"><div class="card-body">
                ...
                <div class="galeria-grid">
                    @foreach($negocio->fotos_galeria as $foto)
                        <img src="{{ asset('storage/' . $foto) }}" class="galeria-img" alt="">
                    @endforeach
                </div>
            </div></div>
            @endif
            <div class="card"><div class="card-body">
                ...
                <div id="lista-servicios">
                    ...
                    <div class="servicio-card" data-nombre="{{ strtolower($servicio->nombre) }}">
                        <div style="display:flex;">
                            @if($servicio->foto)
                                <img src="{{ asset('storage/' . $servicio->foto) }}" class="servicio-img" alt="">
                            @else
                            ...
                            <div style="padding:14px;flex:1;display:flex;flex-direction:column;justify-content:space-between;min-width:0;">
                                <div>
                                    <div style="font-weight:700;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $servicio->nombre }}</div>
                                    @if($servicio->descripcion)
                                        <div style="font-size:12px;color:#64748b;margin-top:3px;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">{{ $servicio->descripcion }}</div>
                                    @endif
                                </div>
                                <div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px;gap:8px;">
                                    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                                        <span class="price-tag">{{ number_format($servicio->precio,2) }}€</span>
                                        <span class="duration-tag">{{ $servicio->duracion_minutos }} min</span>
                                    </div>
                                    <button class="btn-reservar" onclick="abrirReserva({{ $servicio->id }},'{{ addslashes($servicio->nombre) }}',{{ $servicio->duracion_minutos }},{{ $servicio->empleado_id ?? 'null' }})">Reservar</button>
                                </div>
                            </div>
                        </div>
                        @if($servicio->descripcion_larga)
                        <div style="padding:10px 14px;font-size:12px;color:#64748b;border-top:1px solid #f1f5f9;line-height:1.6;background:#fafcff;">{{ $servicio->descripcion_larga }}</div>
                        @endif
                    </div>
                    @endforeach
                </div>
        ...
        <div class="sidebar-order">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="info-item"><div class="info-icon">📍</div><div><div class="info-label">Dirección</div><div class="info-value">{{ $negocio->direccion }}{{ $negocio->ciudad ? ', '.$negocio->ciudad : '' }}</div></div></div>
                    @endif
                    @if($negocio->telefono_negocio)
                    <div class="info-item"><div class="info-icon">📞</div><div><div class="info-label">Teléfono</div><div class="info-value"><a href="tel:{{ $negocio->telefono_negocio }}" style="color:var(--primary);text-decoration:none;">{{ $negocio->telefono_negocio }}</a></div></div></div>
                    @endif
                    @if($negocio->horario_apertura && $negocio->horario_cierre)
                    <div class="info-item"><div class="info-icon">🕐</div><div><div class="info-label">Horario</div><div class="info-value">{{ $negocio->horario_apertura }} – {{ $negocio->horario_cierre }}</div></div></div>
                    @endif
                    @if($setting?->negocio_email)
                    <div class="info-item"><div class="info-icon">✉️</div><div><div class="info-label">Email</div><div class="info-value"><a href="mailto:{{ $setting->negocio_email }}" style="color:var(--primary);text-decoration:none;">{{ $setting->negocio_email }}</a></div></div></div>
                    @endif
                </div>
                @if($negocio->direccion)
<div id="map">
    <iframe src="https://www.google.com/maps?q={{ urlencode($negocio->direccion . ", " . $negocio->ciudad) }}&output=embed&z=16" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
</div>
@endif
            </div>
        </div>
...
<div class="modal-overlay" id="modal-reserva">
    <div class="modal">
        ...
        <div class="modal-body">
            ...
            <form action="{{ route('negocio.store', $negocio->slug) }}" method="POST" id="form-reserva">
                @csrf
                ...
                <div class="step" id="step2">
                    @if(session("social_cliente") && !session("social_cliente.politica_aceptada"))
                    <div style="position:fixed;inset:0;background:rgba(10,20,40,0.7);z-index:2000;display:flex;align-items:flex-end;justify-content:center;backdrop-filter:blur(4px);">
                        <div style="background:white;border-radius:20px 20px 0 0;width:100%;max-width:560px;max-height:88vh;display:flex;flex-direction:column;">
                            ...
                            <div style="padding:16px 24px;border-top:1px solid #f1f5f9;flex-shrink:0;background:white;">
                                ...
                                <form action="/auth/logout/{{ $negocio->slug }}" method="GET" style="margin-top:8px;">
                    ...
                    <div style="background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;gap:10px;">
                        <div>
                            <div style="font-size:13px;font-weight:700;color:#166534;">{{ session('social_cliente.nombre') }}</div>
                            <div style="font-size:12px;color:#16a34a;">{{ session('social_cliente.email') }}</div>
                        </div>
                        <a href="/auth/logout/{{ $negocio->slug }}" style="margin-left:auto;font-size:11px;color:#94a3b8;text-decoration:none;">Cambiar</a>
                    </div>
                    <input type="hidden" name="cliente_id" value="{{ session('social_cliente.id') }}">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        <div class="form-group"><label style="font-size:13px;font-weight:600;color:#475569;display:block;margin-bottom:6px;">Nombre *</label><input class="input-field" type="text" name="nombre" required placeholder="Tu nombre" value="{{ session('social_cliente.nombre') }}"></div>
                    ...
                    <div class="form-group"><label style="font-size:13px;font-weight:600;color:#475569;display:block;margin-bottom:6px;">Email *</label><input class="input-field" type="email" name="email" required value="{{ session('social_cliente.email') }}"></div>
...
function cargarOcupacion(mes,cb){
    fetch(`/reserva/{{ $negocio->slug }}/ocupacion?mes=${mes}`)
...
function cargarSlots(fecha){
    ...
    document.getElementById('input-fecha-hora').value='';
    document.getElementById('btn-paso2').disabled=true;
    fetch(`/reserva/{{ $negocio->slug }}/disponibilidad?fecha=${fecha}&servicio_id=${servicioActual.id}&empleado_id=${servicioActual.empleadoId||''}`)
...
function enviarOtp(){
    ...
    fetch("/auth/otp/enviar/{{ $negocio->slug }}",{
        method:"POST",
...
function verificarOtp(){
    ...
    fetch("/auth/otp/verificar/{{ $negocio->slug }}",{
        method:"POST",
        headers:{"Content-Type":"application/json","X-CSRF-TOKEN":"{{ csrf_token() }}"},
...
function aceptarPolitica(){
    ...
    fetch("/auth/politica/{{ $negocio->slug }}", {
        method: "POST",
        headers: {"Content-Type":"application/json", "X-CSRF-TOKEN":"{{ csrf_token() }}"},
        body: JSON.stringify({})
    }).then(r=>r.json()).then(d=>{
...
function guardarYGoogle(){
    ...
    window.location.href = "/auth/google/redirect/{{ $negocio->slug }}";
}
