    ...
    <div class="card" style="padding:32px; max-width:640px;">
        ...
        <form action="{{ route('clientes.update', $cliente) }}" method="POST">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
                ...
                <div>
                    <label>Email</label>
                    <input type="email" name="email" value="{{ old('email', $cliente->email) }}" placeholder="juan@email.com">
                </div>

                <div>
                    <label>Teléfono</label>
                    <input type="text" name="telefono" value="{{ old('telefono', $cliente->telefono) }}" placeholder="+34 600 000 000">
                </div>

                <div style="grid-column:span 2;">
                    <label>Notas</label>
                    <textarea name="notas" rows="3" placeholder="Observaciones sobre el cliente...">{{ old('notas', $cliente->notas) }}</textarea>
                </div>
            </div>
